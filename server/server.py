...
print "Waiting for client key in password..."					#send message via tcp socket (server->client)

pass_wd = conn.recv(1024)							#receive password to enable transferring file
if pass_wd == "unix123":							#if password correct
	conn.send("confirm")							#send confirm to client 
	print colored("Password correct!Client start to transfer file","red")
else:										#if password wrong
	print colored("Wrong password.Exit the program","red")
