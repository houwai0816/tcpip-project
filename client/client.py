...
#s.recv(1024)										#print the data receive from server after connect
									
#print s.recv(1024)									#print the data receive from server after connect

password = getpass.getpass('Enter password to connect server for transfer file:')	#let user input the password file, which the inpude 												field is hidden.
s.send(password)							                #Send the entered password to server

confirm = s.recv(1024)
if confirm == 'confirm':								#If clause to match the password with the server
	print colored('You are now ready to transfer file to server.\n','cyan')
	...
	print colored('The file is transferred to server.','green') 			                #print message

else:											#else clause when password is not match with server
